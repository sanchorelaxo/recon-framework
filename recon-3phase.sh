...
# Automated reconnaissance workflow with interactive parameters and parallel execution
...
run_phase2() {
    ...
    if [ "$RESUME_MODE" = true ] && [ -f "$phase2_log" ]; then
        ...
        grep -q "^naabu" "$phase2_log" && skip_naabu=true
        grep -q "^httpx" "$phase2_log" && skip_httpx=true
        grep -q "^httprobe" "$phase2_log" && skip_httprobe=true
    fi
    ...
    if [ "$PHASE2_NAABU" = true ] && [ "$skip_naabu" = false ]; then
        ...
        (
            naabu -l "$PHASE2_HOSTS" -p - -c 50 -silent -o "${RESULTS_DIR}/phase2_naabu_ports.txt" 2>&1 | tee -a "$phase2_log"
        ) &
        TOOL_PIDS["naabu"]=$!
...
run_phase3() {
    ...
    if [ "$PHASE3_KATANA" = true ]; then
        ...
        (
            cat "$PHASE3_URLS" | katana -d "$PHASE3_KATANA_DEPTH" -silent -o "${RESULTS_DIR}/phase3_katana_urls.txt" 2>&1 | tee -a "$phase3_log"
        ) &
        TOOL_PIDS["katana"]=$!
